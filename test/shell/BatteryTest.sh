...
do
	...
	if [ $times -gt $2 ];then
			...
			adb shell setprop persist.sys.test ShellKillBatteryIsdone
			KillProcess $1
			break
	fi	
result=$(adb shell getprop persist.sys.test)
...
# battery testing 
cd $NativeDir
adb install -r BatteryTest.apk
PKGName="com.compal.batterytest"
ActivityName="BatteryTestActivity"
Domain="Battery"
...
adb shell setprop persist.sys.test ShellInstallBattery
