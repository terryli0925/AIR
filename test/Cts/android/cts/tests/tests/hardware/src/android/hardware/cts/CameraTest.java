...
//yyyyyyyyy
    private void testTakePicByCamera() throws Exception {
        Size pictureSize = mCamera.getParameters().getPictureSize();
        //assertTrue(focusModes != null && focusModes.size() != 0);
        List<String> focusModes = /*parameters*/mCamera.getParameters().getSupportedFocusModes();
		if(focusModes != null && focusModes.size() != 0){
			if (focusModes.contains(Parameters.FOCUS_MODE_AUTO) || focusModes.contains(Parameters.FOCUS_MODE_MACRO) ) {
				mCamera.autoFocus(mAutoFocusCallback);
				assertTrue(waitForFocusDone());
			}
		}
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    @UiThreadTest  
    public void testTakePicRearSmoothMaxZoom() throws Exception {
        int nCameras = Camera.getNumberOfCameras();
        ...
        for (int id = 0; id < nCameras; id++) {
            ...
            if (info.facing == CameraInfo.CAMERA_FACING_BACK) {
				testTakePicSmoothZoomByCamera(id, 1);
            }
        }
    }
    @UiThreadTest  
    public void testTakePicRearSmoothMinZoom() throws Exception {
        int nCameras = Camera.getNumberOfCameras();
        CameraInfo info = new CameraInfo();
        for (int id = 0; id < nCameras; id++) {
            Log.v(TAG, "Camera id=" + id);
            Camera.getCameraInfo(id, info);
            if (info.facing == CameraInfo.CAMERA_FACING_BACK) {
				testTakePicSmoothZoomByCamera(id, 0);
            }
        }
    }
    @UiThreadTest  
    public void testTakePicFrontSmoothMaxZoom() throws Exception {
        int nCameras = Camera.getNumberOfCameras();
        CameraInfo info = new CameraInfo();
        for (int id = 0; id < nCameras; id++) {
            Log.v(TAG, "Camera id=" + id);
    ...
    @UiThreadTest  
    public void testTakePicFrontSmoothMinZoom() throws Exception {
        int nCameras = Camera.getNumberOfCameras();
        CameraInfo info = new CameraInfo();
        for (int id = 0; id < nCameras; id++) {
            Log.v(TAG, "Camera id=" + id);
    ...
    private void testTakePicSmoothZoomByCamera(int id, int xZoom) throws Exception {
        initializeMessageLooper(id); //yyyyyyy
        ...
        for (Size size: parameters.getSupportedPreviewSizes()) {
            ...
            mCamera.takePicture(mShutterCallback, mRawPictureCallback,
                                mJpegPictureCallback);
            waitForSnapshotDone();
			break;// just do once -- one supported preview size
        }
