...
do
	...
	if [ $times -gt $2 ];then
			...
			adb shell setprop persist.sys.test ShellKillBluetoothIsdone
			KillProcess $1
			break
	fi	
result=$(adb shell getprop persist.sys.test)
...
# sensor testing 
cd $NativeDir
adb install -r BluetoothTest.apk
PKGName="com.compal.bluetoothtest"
...
adb shell setprop persist.sys.test ShellInstallBluetooth
