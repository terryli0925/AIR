...
do
	...
	if [ $times -gt $2 ];then
			...
			adb shell setprop persist.sys.test ShellKillTelephonyIsdone
			KillProcess $1
			break
	fi	
result=$(adb shell getprop persist.sys.test)
...
# sensor testing 
cd $NativeDir
adb install -r TelephonyTest.apk
PKGName="com.compal.telephonytest"
...
adb shell setprop persist.sys.test ShellInstallTelephony
