...
do
	...
	if [ $times -gt $2 ];then
			...
			adb shell setprop persist.sys.test ShellKillGpsIsdone
			KillProcess $1
			break
	fi	
result=$(adb shell getprop persist.sys.test)
...
# sensor testing 
cd $NativeDir
adb install -r GpsTest.apk
PKGName="com.compal.gpstestcase"
...
adb shell setprop persist.sys.test ShellInstallGps
